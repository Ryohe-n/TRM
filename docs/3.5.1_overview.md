...
@{Key=3.5.1; Title=Overview; Body=same architecture, as follows: 1.
...
then the control register of that channel is programmed. If the control register is programmed first, the current parameters of the DMA would be considered as the programmed values.}(@{Key=3.5.1; Title=Overview; Body=same architecture, as follows: 1.
- General-Purpose Central DMA controller (GPC-DMA, also referred to as GPCDMA) placed on
the Control Fabric. It has 32 fully programmable independent channels which can be programmed by different masters.
- DMA engine as part of the BPMP subsystem (BPMP-DMA). It has four fully programmable
independent channels.
...
address switching on the MMIO.
- The parameters of the channel must be programmed first (base address, wrap-around, etc.),
then the control register of that channel is programmed. If the control register is programmed first, the current parameters of the DMA would be considered as the programmed values.}.Body)
